...
export const checkIfChatbotExists = async (userid: string, botid: string) => {
    const res: any = { success: true }
    try {
        const { data: userdata } = await supabaseAdmin
            .from('bots')
            .select('id, req_per_min, base_prompt')
            .eq('user_id', userid).eq('uuid', botid)
            .throwOnError();
...
export const saveConversationApi = async (visitor_id: string, chat: any, botid: string, userid: string, geo: any) => {
    const response: any = { success: true }
    try {
        const { data: userConversations } = await supabaseAdmin
            .from("conversations")
            .select("id")
            .eq("visitor_id", visitor_id).limit(1)
            .throwOnError();
        if(userConversations && userConversations?.length > 0) {
            const { data: res } = await supabaseAdmin
            .from('conversations')
            .update({chat_data: chat, geo: geo})
            .eq("visitor_id", visitor_id).eq("bot_id", botid).eq("user_id", userid)
            .select("*")
            .throwOnError()
            ...
            const { data: res } = await supabaseAdmin
            .from('conversations')
            .insert({chat_data: chat, visitor_id: visitor_id, bot_id: parseInt(botid), user_id: userid, geo: geo})
            .select("*")
            .throwOnError()
